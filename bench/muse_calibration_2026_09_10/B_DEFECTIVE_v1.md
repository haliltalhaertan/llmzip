...
**Prepared by:** Continuity Lead
**Binding when** committed with an agreeing `.sha256` sidecar and covered by a
pre-run seal, on Head Researcher authorization.
**Repository path (proposed):** `docs/v52/V52_TWELVE_BYTE_BASELINE_PREREG_2026-09-09.md`
...
SIGN96      LoCoMo      Fractional R@3 == 0.23654714666441054  (±1e-9)
...
|---|---|---|
| B1 | `Δ ≥ +2.0` with 95% CI excluding 0, **on both benchmarks** | `[SIGN96 COMPETITIVE AT MATCHED BUDGET]` |
| B2 | `\|Δ\| < 2.0` on both benchmarks | `[PARITY AT MATCHED BUDGET]` |
| B3 | `Δ ≤ −3.0` with 95% CI excluding 0, **on both benchmarks** | `[SIGN96 SUPERSEDED AT MATCHED BUDGET]` |
| B4 | benchmarks disagree in direction | `[NOT ESTABLISHED — BENCHMARK DISAGREEMENT]` |
...
**Negative-result commitment.** B2, B3 and B4 are written up and published in the
same form as B1. No band is a reason not to report.
...
## 12. Open item for the Head Researcher

Arms 4–6 require third-party implementations (RaBitQ, extended RaBitQ, OPQ/PQ). The
programme's environment lock and audit-hook policy must be extended to cover them,
or reference implementations must be vendored and hashed. **This is a dependency
decision and is not taken in this draft.**
