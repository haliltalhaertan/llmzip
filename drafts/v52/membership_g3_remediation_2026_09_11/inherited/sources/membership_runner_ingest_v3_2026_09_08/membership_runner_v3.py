...
On content, stated as a tested surface rather than as an absolute - v2 said "no value is interpolated
into any message in this module" and the closure check falsified it with NINE paths, so the claim is
...
`UnsafeErrorField`. The tested surface is every exception this module raises, stdout, stderr, the
exception chain, and every file it writes. NOT claimed: that content cannot escape by some other
route - a caller that prints what it is handed defeats all of it, and no module can prevent that.
...
RUNNER_VERSION = "membership_runner v3 2026-09-08"
