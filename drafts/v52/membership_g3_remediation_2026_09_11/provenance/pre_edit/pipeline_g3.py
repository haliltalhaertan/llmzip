...
def _verified_core_bytes(raw):
    """Accept the bound blob or only Git's CRLF checkout translation of that blob."""
    if hashlib.sha256(raw).hexdigest() == CORE_HASH:
        return raw
    if b'\r\n' in raw and b'\r' not in raw.replace(b'\r\n', b''):
        canonical = raw.replace(b'\r\n', b'\n')
        if hashlib.sha256(canonical).hexdigest() == CORE_HASH:
            return raw
...
def _coordinatewise_mismatch(X, q):
    return np.not_equal(np.asarray(X) >= 0, np.asarray(q) >= 0)


def coordinatewise_signed_certificate(X, V):
    """Compare per-coordinate mismatch bits before any Hamming aggregation."""
    X, V = np.asarray(X), np.asarray(V)
    require(X.ndim == 2 and X.shape[1] == 96 and np.isfinite(X).all(), 'E-M-033: canary archive')
    require(V.ndim == 2 and V.shape[1] == 96 and np.isfinite(V).all(), 'E-M-034: canary queries')
    for q in V:
        baseline = _coordinatewise_mismatch(X, q)
        for mask in SIGNED_CANARY_MASKS:
            signs = _signs_from_mask(mask)
            transformed = _coordinatewise_mismatch(X * signs, q * signs)
            require(np.array_equal(baseline, transformed), 'E-M-031: signed permutation control')
