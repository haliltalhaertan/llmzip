...
def validate_ingested(ingested, anchors):
    require(type(ingested) is dict, 'E-M-034: ingestion schema')
    require({'benchmark', 'cohort_ids', 'questions_with_empty_gold'} <= set(ingested),
            'E-M-034: ingestion schema')
    ids = ingested['cohort_ids']
    require(type(ids) is list and len(ids) > 0, 'E-M-035: empty or invalid cohort')
    require(all(type(q) is str and q.strip() == q and q.casefold() not in
                {'', 'none', 'nan', 'null', 'na', 'n/a', 'nil', '-', '--', '?'} for q in ids),
            'E-M-018: identifiers')
    require(len(ids) == len(set(ids)), 'E-M-019: duplicate identifiers')
    require(type(ingested['questions_with_empty_gold']) is list
            and ingested['questions_with_empty_gold'] == [], 'E-M-026: empty gold in cohort')
    require(type(anchors) is dict and set(anchors) == set(ids), 'E-M-027: anchor coverage')
    benchmark = ingested['benchmark']
    require(type(benchmark) is str and benchmark in ('LoCoMo', 'LongMemEval'), 'E-M-028: benchmark')
    groups = []
    if benchmark == 'LoCoMo':
        require(type(ingested.get('conversations')) is dict and bool(ingested['conversations']), 'E-M-034: ingestion schema')
        ordinals = set()
        for group in ingested['conversations'].values():
            require(type(group) is dict and {'index', 'units', 'questions'} <= set(group), 'E-M-034: ingestion schema')
            require(type(group['index']) is int and group['index'] >= 0 and group['index'] not in ordinals, 'E-M-010: archive ordinal')
            ordinals.add(group['index'])
            require(type(group['questions']) is dict, 'E-M-034: ingestion schema')
            if group['questions']:
                groups.append((group['units'], group['questions']))
    else:
        require(type(ingested.get('questions')) is dict, 'E-M-034: ingestion schema')
        for qid, question in ingested['questions'].items():
            require(type(question) is dict and 'units' in question, 'E-M-034: ingestion schema')
            groups.append((question['units'], {qid: question}))
    seen = []
    for units, questions in groups:
        require(type(units) is list and bool(units), 'E-M-034: ingestion schema')
        require(all(type(u) is dict and type(u.get('text')) is str for u in units), 'E-M-034: ingestion schema')
        for qid, question in questions.items():
            require(type(qid) is str and qid in ids, 'E-M-036: question coverage')
            require(type(question) is dict and type(question.get('text')) is str and 'gold_rows' in question, 'E-M-034: ingestion schema')
            if benchmark == 'LongMemEval':
                validate_gold(question['gold_rows'], len(units))
            # LoCoMo raw gold is not consumed: corrected gold is resolved before fitting.
            seen.append(qid)
    require(len(seen) == len(ids) and set(seen) == set(ids), 'E-M-036: question coverage')
    return ids


def assemble_in_memory(ingested, native_anchors, *, source_ids=None,
                       locomo_raw_evidence=None, locomo_corrections=None,
                       synthetic_counts=None):
    """Validated in-memory bridge; callers authenticate sources outside this module.

    Only synthetic invocation authorized. Does not activate legacy file ingestion.
    Original structures are not mutated; corrected gold is used in new question dicts.
    """
    ids = validate_ingested(ingested, native_anchors)
    benchmark = ingested['benchmark']
    source_plan = None
    if benchmark == 'LoCoMo':
        maps = {}
        for c in ingested['conversations'].values():
            if not c['questions']:
                continue
            require(all(type(u.get('dia_id')) is str for u in c['units']), 'E-M-034: ingestion schema')
            refs = [u['dia_id'] for u in c['units']]
            require(len(set(refs)) == len(refs), 'E-M-037: duplicate memory identifiers')
            maps.update({q: dict(zip(refs, range(len(refs)))) for q in c['questions']})
        resolved = contracts.resolve_locomo_gold(ids, locomo_raw_evidence, locomo_corrections, maps)
        archives = [(c['index'], c['units'],
                     {q: dict(v, **resolved[q]) for q, v in c['questions'].items()})
                    for c in ingested['conversations'].values() if c['questions']]
    else:
        require(synthetic_counts is None or (type(synthetic_counts) is tuple and len(synthetic_counts) == 2
                and all(type(n) is int and n > 0 for n in synthetic_counts)), 'E-M-038: synthetic counts')
        counts = {} if synthetic_counts is None else dict(expected_source=synthetic_counts[0], expected_primary=synthetic_counts[1])
        source_plan = contracts.longmemeval_plan(source_ids, ids, **counts)
        archives = [(row['archive_ordinal'], ingested['questions'][row['question_id']]['units'],
                     {row['question_id']: ingested['questions'][row['question_id']]}) for row in source_plan]
    records, diagnostics = [], []
    ...
    return {'records': records, 'diagnostics': diagnostics, 'source_plan': source_plan}
