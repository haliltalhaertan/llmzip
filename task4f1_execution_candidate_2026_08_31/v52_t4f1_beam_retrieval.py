...
def verify_execution_seal(path: Path, script_path: Path) -> tuple[dict[str, Any], str]:
    ...
    if seal.get("schema") != "V52_T4F1_EXECUTION_CANDIDATE_SEAL_V1":
...
def evaluate_archive(
    ...
    metadata = {
        "schema": "V52_T4F1_ARCHIVE_RESULT_META_V1",
...
def verify_existing_archive(output_dir: Path, archive_id: str, expected_questions: int) -> bool:
    safe_id = archive_id.replace("::", "__")
    csv_path = output_dir / "archives" / f"{safe_id}.csv"
    meta_path = output_dir / "archives" / f"{safe_id}.meta.json"
    ...
    if (
        metadata.get("schema") != "V52_T4F1_ARCHIVE_RESULT_META_V1"
...
def finalize_results(
    ...
    for archive_id in sorted(by_archive):
        if not verify_existing_archive(output_dir, archive_id, len(by_archive[archive_id])):
    ...
    manifest = {
        "schema": "V52_T4F1_POST_RUN_MANIFEST_V1",
        "task": TASK,
        "status": "COMPLETE_PENDING_INDEPENDENT_RESULT_AUDIT",
        "script_sha256": sha256_file(script_path),
        "execution_candidate_seal_sha256": sha256_file(execution_seal_path),
        "run_authorization_sha256": sha256_file(authorization_path),
...
def preflight(args: argparse.Namespace, script_path: Path) -> None:
    ...
    report = {
        "schema": "V52_T4F1_IMPLEMENTATION_PREFLIGHT_V1",
        "status": "PASS",
...
def run_archives(args: argparse.Namespace, script_path: Path) -> None:
    ...
    verify_execution_seal(args.execution_seal, script_path)
    eligible, cohort_summary = load_and_verify_cohort(args.cohort)
    ...
    for archive_id in requested:
        if verify_existing_archive(args.output_dir, archive_id, len(by_archive[archive_id])):
            if args.resume:
        ...
        rows, metadata = evaluate_archive(args.corpus_root, archive_id, by_archive[archive_id])
...
def main() -> None:
    ...
    if args.mode == "run":
        ...
        verify_beam_checkout(args.corpus_root, cohort_summary["archive_ids"], args.beam_manifest)
        finalize_results(args.output_dir, eligible, args.authorization, script_path, args.execution_seal)
