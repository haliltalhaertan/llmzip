...
| Task 4F1 scientific preregistration | **SEALED** 2026-09-04 (Seal V2; V1 blocked) |
...
**The scientific preregistration is SEALED at Seal V2** (2026-09-04): `docs/v52/task4f1/TASK4F1_PREREGISTRATION_SEAL_V2_2026-09-04.json` (SHA256 `9ed480f4…`, with a `.sha256` sidecar). Verify with `python -B tools/verify_preregistration_seal.py`, and check the verifier itself with `python -B tools/test_verify_preregistration_seal.py` (16 negative controls). **Seal V1 (`c9e06195…`) is BLOCKED and superseded** — it carried hand-transcribed tier labels that dropped `500K` and invented a `5M` tier, and its verifier never tested them. V1 and its sidecar are preserved byte-unchanged as historical defective artifacts; do not edit or delete them. V2 derives every value from bytes via `tools/build_preregistration_seal_v2.py`. The seal binds the approved draft `5e618981…`, approved by the Head Researcher (`hr/rereview-t4f1-prereg-2026-09-04` @ `19d9cbb`) and by an independent exact-byte co-chair (`cochair/exact-byte-approval-t4f1-prereg-2026-09-04` @ `5299cc1`). **Sealing is not a run authorization.** Run stays BLOCKED, no production authorization exists, outcome access stays FORBIDDEN. Any scientific-content change needs both approvals again. The remaining blocker is the execution track: the V7 package audit, plus the pre-run exact-rational `D_t` sign-classification condition.
