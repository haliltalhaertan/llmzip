...
## Canary specification (V4)

The real-archive representation canary on `100K::12` asserts:
...
Conditions 3 and 4 replace the V3 bit-exact float digests. Condition 5 is the guard
that makes 3 and 4 safe: it fails closed if the representation ever drifts close
enough to zero that a sign code could flip between conformant environments.

The canary loads only `chat.json`. It does not open probing questions, does not load
gold, does not rank and does not compute any retrieval-quality metric.
