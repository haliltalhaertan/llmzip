...
"""verify.py — exact rank-change / top-K stability certificates.

Scoring law (joint doc+query scaling, fixed group G, z=t^2, common positive
query norm removed):  f_i(z) = (a_i + b_i z) / sqrt(u_i + v_i z),
domain u_i + v_i z > 0. All certificates are exact rational statements about
the exact rational reading of the coefficients below. They do NOT transfer to
floating-point BLAS evaluation order (see section N).

Run (no installs, no network):
  OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 PYTHONDONTWRITEBYTECODE=1 \\
    /home/mdp/muse-work/ml-python -B verify.py
Writes results.json next to this file. Exit nonzero on any failure.
numpy is used ONLY in clearly marked informational (non-certificate) spots;
the certificate path is pure stdlib (fractions).
"""
import json
import math
import os
...
def run_all():
    ...
    if found11 is None:
        check("S7-1plus1-unresolved", True, "evals=%d, no witness in grid" % evals)
    else:
