"""V52 static-storage external binding preflight V4.

Repairs the adapter-boundary findings of the independent V3 audit at
audit/v52-static-storage-plan-prep-v3-independent-2026-09-12 @ b06247df2124edc6953aacf0f7eb69eed99ac1e0.
V3 bytes at 3fab24bce46203348935492ba8276b9920aea898 are not rewritten.
...
Repaired here: AUD-006 (non-regular file hangs the open), AUD-007 (no binding between
the guarded plan and the preflighted plan), AUD-002 (unbounded corruption offset),
AUD-005 (zero-byte artifact without absence evidence), AUD-009 (OSError/ValueError
escaping as non-PreflightError), AUD-010 (version fields accept bool/float),
AUD-011 (two copies bound to one file).

NOT repaired here, because it belongs to the plan guard and the plan schema:
AUD-001 (no external anchor for N_i) and AUD-008 (denominator rule yields a menu).
This module therefore still refuses to select a denominator; it reports states only.
"""
...
MAX_ENTITIES = 1024                             # AUD-004: match the parent guard
MAX_DEPTH = 24
...
def read_bytes(path, limit, where, budget=None):
    """AUD-006: never block on a non-regular file, and never follow a symlink.

    The handle is opened O_NOFOLLOW|O_NONBLOCK and fstat'ed, so the file that is
    actually read is the file that was checked. AUD-009: every OSError becomes a
    PreflightError.
    """
    try:
    ...
    try:
        ...
        while True:
            ...
            if not chunk:
                break
            raw += chunk
            need(len(raw) <= limit, f"{where}: file byte limit")
    finally:
        os.close(fd)
...
def verify_fixture_artifact(binding_dir, plan_fixture, binding, budget):
...
def verify_physical_artifact(binding_dir, plan_copy, binding, populations, seen_inodes, budget):
    ...
    # AUD-011: two declared copies must not resolve to one file on disk.
    try:
        st = os.stat(str(artifact), follow_symlinks=False)
    except OSError as e:
        raise PreflightError(f"physical locator: cannot stat artifact: {e.__class__.__name__}") from e
    identity = (st.st_dev, st.st_ino)
    if identity in seen_inodes:
        raise PreflightError(
            f"physical copy {plan_copy['id']}: distinct copies must not resolve to one file "
            f"(already bound by {seen_inodes[identity]})")
    seen_inodes[identity] = plan_copy["id"]
...
def preflight(plan_path, expected_plan_sha256, expected_contract_sha256,
              fixture_bindings_path, expected_fixture_bindings_sha256,
              physical_bindings_path, expected_physical_bindings_sha256,
              guarded_plan_sha256=None, guarded_contract_sha256=None):
    """AUD-007: the caller must state which plan the parent guard validated.

    `guarded_plan_sha256` and `guarded_contract_sha256` are what
    `measurement_plan_guard.load_plan` returned. When either is supplied it must equal
    what this preflight authenticates, so guarding plan A and preflighting plan B is a
    refusal rather than a silent divergence. Both are required together.
    """
    need((guarded_plan_sha256 is None) == (guarded_contract_sha256 is None),
         "guarded plan and contract digests must be supplied together")
    if guarded_plan_sha256 is not None:
    ...
    exact_version(pdoc["version"], 3, "physical bindings version")
    ...
    vf = tuple(verify_fixture_artifact(fdir, plan_fixtures[i], fixture_bindings[i], budget)
