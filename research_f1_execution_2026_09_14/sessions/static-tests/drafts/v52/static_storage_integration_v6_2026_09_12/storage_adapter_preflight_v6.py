"""V52 static-storage external binding preflight V6.
...
V6 closes the gap that survived three versions. V3 had no binding between the plan guard
and the preflight; V4 added one that defaulted off; V5 split the anchoring into a second
module and connected them with a sentence, and no code ever called it. Here the anchored
denominators are a REQUIRED argument, cross-checked against the plan, and one value is
emitted per copy instead of the menu. A caller that skips the anchor guard cannot call
...
# Platform-optional open flags. V5 wrote os.O_NOFOLLOW bare while using getattr for
# O_CLOEXEC on the same line; on Windows that is an AttributeError, so the module broke
# its own promise that every refusal is a PreflightError. Absent flags degrade to 0.
_O_NOFOLLOW = getattr(os, "O_NOFOLLOW", 0)
_O_NONBLOCK = getattr(os, "O_NONBLOCK", 0)
_O_CLOEXEC = getattr(os, "O_CLOEXEC", 0)
_O_BINARY = getattr(os, "O_BINARY", 0)          # Windows needs this; no-op elsewhere
SYMLINK_FLAG_AVAILABLE = bool(_O_NOFOLLOW)
NONBLOCK_FLAG_AVAILABLE = bool(_O_NONBLOCK)
...
def read_bytes(path, limit, where, budget=None):
    ...
    O_NOFOLLOW, where the platform has it, applies to the final component of the path as
    given. Artifact locators reach here already resolved by safe_child, which follows
    symlinks and then requires the result to stay under the binding directory, so a
    symlink to a regular file inside the directory is accepted by design.

    Where the platform lacks O_NOFOLLOW or O_NONBLOCK, those degrade to zero rather than
    raising AttributeError. A FIFO is then refused by the PRE-OPEN stat, not by the
    fstat: without O_NONBLOCK the open blocks before any fd exists, so a post-open check
    can never run. An earlier draft of this module claimed the fstat was the portable
    refusal; simulating a platform without the flags hung it, which is the AUD-006 hang
    returning by another route. The pre-open stat refuses; the post-open fstat then
    proves the fd is the same inode that was checked, so the TOCTOU window stays closed.
    SYMLINK_FLAG_AVAILABLE and NONBLOCK_FLAG_AVAILABLE report which flags were compiled
    in, so a caller can record the weaker guarantee instead of assuming it.
    """
    # Refuse before opening. Without O_NONBLOCK the open itself blocks on a FIFO, so a
    # post-open fstat can never run - which is exactly the AUD-006 hang, returning on any
    # platform that lacks the flag. The pre-open stat is the portable refusal; the
    # post-open fstat below proves the fd is the same object that was checked.
    try:
    ...
    need(stat.S_ISREG(pre.st_mode), f"{where}: artifact must be a regular file")
    ...
    try:
        ...
        need((st.st_dev, st.st_ino) == (pre.st_dev, pre.st_ino),
             f"{where}: artifact changed identity between the check and the open")
...
class VerifiedPhysicalCopy:
    ...
    denominator: tuple          # (population_id, count) chosen by the anchor guard
...
def verify_physical_artifact(binding_dir, plan_copy, binding, populations, anchored, seen_inodes, budget):
    ...
    for pop_id in plan_copy["population_ids"]:
        ...
        integer(populations[pop_id]["count"], f"population {pop_id} count")

    # The anchor guard chose this; the preflight only checks the choice is consistent with
    # the plan it just authenticated. Neither module can produce a denominator alone.
    chosen = anchored[plan_copy["id"]]
    need(type(chosen) is tuple and len(chosen) == 2,
         f"physical copy {plan_copy['id']}: anchored denominator must be (population_id, count)")
    pop_id, count = chosen
    need(pop_id in plan_copy["population_ids"],
         f"physical copy {plan_copy['id']}: anchored population {pop_id} is not bound to this copy")
    integer(count, f"physical copy {plan_copy['id']} anchored denominator", 1)
    need(populations[pop_id]["count"] == count,
         f"physical copy {plan_copy['id']}: anchored denominator {count} disagrees with the plan's "
         f"population count {populations[pop_id]['count']}")
    return VerifiedPhysicalCopy(plan_copy["id"], raw, binding["artifact_sha256"], len(raw),
                                (pop_id, count))
...
def preflight(plan_path, expected_plan_sha256,
    ...
    """
    budget = _Budget()
    need(type(anchored_denominators) is dict,
         "anchored_denominators must be the mapping archive_anchor_guard returned")
    ...
    need(set(anchored_denominators) == set(plan_copies),
         "anchored denominator coverage must equal the plan's physical-copy roster; run "
         "archive_anchor_guard.verify_plan_against_anchor on this exact plan first")
    ...
    vc = tuple(verify_physical_artifact(pdir, plan_copies[i], physical_bindings[i], populations,
                                        anchored_denominators, seen_inodes, budget)
