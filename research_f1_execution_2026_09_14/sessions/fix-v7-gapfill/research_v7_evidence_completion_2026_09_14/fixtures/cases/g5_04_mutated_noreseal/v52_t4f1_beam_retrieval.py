...
# V4: the load-bearing canary expectations are SIGN CODES, not raw float bytes.
# Raw float digests were shown to depend on BLAS/LAPACK kernel dispatch (V3 audit, Gate 3):
# four distinct digests across six OPENBLAS_CORETYPE settings on identical code, data and
# locked package versions. The sign codes that actually drive Hamming retrieval were identical
# across every one of those dispatches, with the nearest entry to a sign flip roughly 3.4e6
# times farther from zero than the largest cross-dispatch discrepancy.
EXPECTED_REAL_CANARY_ARCHIVE_SIGN_SHA256 = "7365b6c4ba7753ee5431f89816c3151a2618f475415024d8932c839609ded5b5"
EXPECTED_REAL_CANARY_QUERY_SIGN_SHA256 = "5e40a5d1f0d33bf16c4005ed8aa172464dd1fb205da983f5373c9940f4ccad2b"
# Sign-stability guard: the smallest |value| must stay far above the observed cross-dispatch
# numerical spread (~1.2e-13), otherwise a sign code could flip between conformant environments.
CANARY_SIGN_MARGIN = 1e-9
...
def verify_environment() -> dict[str, Any]:
    ...
    return {
        "versions": observed,
        "thread_environment": dict(THREAD_LOCK),
        "numerical_backend": describe_numerical_backend(),
    }


def describe_numerical_backend() -> dict[str, Any]:
    """Record, but do not gate on, the BLAS/LAPACK build and CPU kernel dispatch.

    The V3 audit showed raw float digests vary with this dispatch. V4 no longer binds
    raw floats, so this is provenance for a later auditor rather than a lock condition.
    """
    ...
    try:
        ...
        backend["blas"] = {
            name: {key: info.get(key) for key in ("name", "version")}
            for name, info in dependencies.items()
            if name in ("blas", "lapack")
        }
    except Exception as exc:  # provenance only; never fail the run on introspection
        backend["blas"] = f"unavailable: {type(exc).__name__}"
    return backend
...
def sign_code_sha256(value: np.ndarray) -> str:
    """Digest the sign codes that drive Hamming retrieval, not the raw float bytes.

    Sign codes are invariant across BLAS/LAPACK kernel dispatch; raw float bytes are not.
    """
    codes = np.ascontiguousarray(np.asarray(value, dtype=np.float64) >= 0)
    return hashlib.sha256(np.packbits(codes).tobytes()).hexdigest()
...
def evaluate_archive(
    ...
    metadata = {
        "schema": "V52_T4F1_ARCHIVE_RESULT_META_V4",
...
def verify_existing_archive(
    ...
    if (
        metadata.get("schema") != "V52_T4F1_ARCHIVE_RESULT_META_V4"
...
def real_archive_canary(corpus_root: Path) -> dict[str, Any]:
    ...
    V4 checks dispatch-stable sign codes plus a sign-stability margin instead of
    bit-exact float digests. It still performs no ranking and reads no gold.
    """
    ...
    archive_sign_digest = sign_code_sha256(model.centered96)
    query_sign_digest = sign_code_sha256(query)
    archive_margin = float(np.min(np.abs(model.centered96)))
    query_margin = float(np.min(np.abs(query)))
    checks = {
        "archive_id": "100K::12",
        "archive_units": len(raw_ids),
        "archive_shape": list(model.centered96.shape),
        "query_shape": list(query.shape),
        "archive_sign_sha256": archive_sign_digest,
        "query_sign_sha256": query_sign_digest,
        "expected_archive_sign_sha256": EXPECTED_REAL_CANARY_ARCHIVE_SIGN_SHA256,
        "expected_query_sign_sha256": EXPECTED_REAL_CANARY_QUERY_SIGN_SHA256,
        "archive_sign_margin": archive_margin,
        "query_sign_margin": query_margin,
        "required_sign_margin": CANARY_SIGN_MARGIN,
    ...
    checks["status"] = "PASS" if (
        ...
        and archive_sign_digest == EXPECTED_REAL_CANARY_ARCHIVE_SIGN_SHA256
        and query_sign_digest == EXPECTED_REAL_CANARY_QUERY_SIGN_SHA256
        and archive_margin > CANARY_SIGN_MARGIN
        and query_margin > CANARY_SIGN_MARGIN
    ) else "FAIL"
    if checks["status"] != "PASS":
...
def write_csv(path: Path, rows: Sequence[dict[str, Any]], fieldnames: Sequence[str]) -> None:
    ...
    if path.exists() or temporary.exists():
        raise RuntimeError(f"[BLOCKED - REFUSE DERIVED OUTPUT OVERWRITE] {path.name}")
    ...
    try:
        ...
        raise RuntimeError(f"[BLOCKED - REFUSE DERIVED OUTPUT OVERWRITE] {path.name}") from exc
    temporary.unlink()
...
def write_json_no_replace(path: Path, value: Any) -> None:
    ...
    temporary = path.with_name(path.name + ".tmp")
    if path.exists() or temporary.exists():
        raise RuntimeError(f"[BLOCKED - REFUSE FINALIZATION OVERWRITE] {path.name}")
    with temporary.open("xb") as handle:
        handle.write(canonical_json_bytes(value))
    try:
        os.link(temporary, path)
    except FileExistsError as exc:
        raise RuntimeError(f"[BLOCKED - REFUSE FINALIZATION OVERWRITE] {path.name}") from exc
    temporary.unlink()
...
def finalize_results(
    ...
    manifest_path = output_dir / "V52_T4F1_POST_RUN_MANIFEST.json"
    finalization_paths = (seed_path, question_path, aggregate_path, manifest_path)
    reserved_paths = (*finalization_paths, *(path.with_name(path.name + ".tmp") for path in finalization_paths))
    existing = sorted(path.name for path in reserved_paths if path.exists())
    if existing:
        raise RuntimeError(f"[BLOCKED - REFUSE FINALIZATION OVERWRITE] {existing}")
    ...
    cell_payloads: dict[tuple[str, str, str, int], tuple[tuple[str, ...], tuple[int, ...]]] = {}
    for row in trial_rows:
        ...
        if (
            ...
            or any(not isinstance(raw_id, str) for raw_id in retrieved)
            ...
            or any(
                ...
                or distance < 0
                or distance > MIXED_DIM
                for distance in distances
            )
        ):
            raise RuntimeError(f"[BLOCKED - TOP3 RESULT SCHEMA] {cell}")
        try:
            canonical_retrieved = [canonical_raw_id(raw_id) for raw_id in retrieved]
            frozen_gold = {canonical_raw_id(raw_id) for raw_id in frozen["gold_source_ids_parsed"]}
        except ValueError as exc:
            raise RuntimeError(f"[BLOCKED - TOP3 OR GOLD ID CANONICALIZATION] {cell}") from exc
        if canonical_retrieved != retrieved or len(frozen_gold) != int(row["gold_count"]):
            raise RuntimeError(f"[BLOCKED - TOP3 OR GOLD ID SEMANTICS] {cell}")
        ...
        expected_metrics = metrics_at_3(retrieved, frozen_gold)
        if (fractional, int(any_value), int(all_value)) != expected_metrics:
            raise RuntimeError(f"[BLOCKED - METRIC GOLD RECOMPUTATION MISMATCH] {cell}")
        cell_payloads[cell] = (tuple(retrieved), tuple(distances))
    ...
    for question_id in expected_by_question:
        for trial in NUISANCE_TRIALS:
            native_payload = cell_payloads[(question_id, "NATIVE_SIGN96", "", trial)]
            for seed in SIGNED_PERM_SEEDS:
                control_payload = cell_payloads[(question_id, "SIGNED_PERM_CONTROL96", str(seed), trial)]
                if control_payload != native_payload:
                    raise RuntimeError(
                        f"[BUG - SIGNED CONTROL EXACT TOP3 OR DISTANCE MISMATCH] {question_id} seed={seed} trial={trial}"
                    )
    ...
    for (question_id, method, seed), values in sorted(by_question_seed.items()):
        ...
        trial_payloads = {
            cell_payloads[(question_id, method, seed, int(value["trial"]))]
            for value in values
        }
        if len(trial_payloads) != 1:
            raise RuntimeError(
                f"[BUG - TRIAL-VARYING TOP3 OR DISTANCE UNDER FIXED PRIORITY] {question_id} {method} {seed}"
            )
        base = {
    ...
    print(f"COMPLETE: all {EXPECTED_ARCHIVES} archives finalized; outcome values were not printed", flush=True)
...
def preflight(args: argparse.Namespace, script_path: Path) -> None:
    ...
    report = {
        "schema": "V52_T4F1_IMPLEMENTATION_PREFLIGHT_V4",
        "status": "PASS",
