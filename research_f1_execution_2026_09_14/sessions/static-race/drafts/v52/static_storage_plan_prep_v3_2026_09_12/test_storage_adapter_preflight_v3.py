...
class V3PreflightTests(unittest.TestCase):
    def setup_case(self,root):
        root=Path(root)
        fx={"schema":"V52_SYNTHETIC_FIXTURE_BUNDLE_V3","transform":{"input_utf8":"hello"},"id_mapping":{"rows":[{"logical_id":"a","offset":0,"payload_utf8":"A"},{"logical_id":"b","offset":1,"payload_utf8":"B"}]},"corruption":{"strategy":"XOR_SINGLE_BYTE","byte_offset":0,"xor_mask":1}}
        ...
        pb={"schema":"v52.physical-copy-bindings","version":3,"physical_copies":[{"physical_copy_id":"pc1","physical_locator":"phys.bin","source_raw_byte_length":len(pr),"artifact_sha256":h(pr),"sharing_denominator_rule":"POPULATION_COUNT"}]}
    ...
    def test_authenticated_bytes_and_zero_semantics(self):
        with tempfile.TemporaryDirectory() as td:
            hs=self.setup_case(td); out=self.run_pf(td,hs); before=out.fixtures[0].raw_bytes; Path(td,"fx.json").write_bytes(b'{"mutated":true}')
    ...
    def test_duplicate_key_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            hs=list(self.setup_case(td)); raw=b'{"schema":"v52.fixture-bindings","schema":"v52.fixture-bindings","version":3,"fixtures":[]}'
            Path(td,"fb.json").write_bytes(raw); hs[1]=h(raw)
            with self.assertRaises(PreflightError): self.run_pf(td,hs)
    def test_nonfinite_rejected(self):
        with tempfile.TemporaryDirectory() as td:
            hs=list(self.setup_case(td)); raw=b'{"schema":"v52.fixture-bindings","version":3,"fixtures":NaN}'
            Path(td,"fb.json").write_bytes(raw); hs[1]=h(raw)
            with self.assertRaises(PreflightError): self.run_pf(td,hs)
