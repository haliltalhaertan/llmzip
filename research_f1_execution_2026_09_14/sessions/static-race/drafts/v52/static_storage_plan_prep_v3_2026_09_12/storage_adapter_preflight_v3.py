...
def validate_fixture_payload(p):
    ...
    fields(p["id_mapping"],"rows","fixture id_mapping"); rows=p["id_mapping"]["rows"]
    need(type(rows) is list and 2<=len(rows)<=MAX_ENTITIES,"id_mapping rows: at least two rows required")
    ids=[]; offsets=[]
    for row in rows:
        fields(row,"logical_id offset payload_utf8","id_mapping row"); text(row["logical_id"],"id_mapping logical_id"); integer(row["offset"],"id_mapping offset"); text(row["payload_utf8"],"id_mapping payload_utf8"); ids.append(row["logical_id"]); offsets.append(row["offset"])
    need(len(ids)==len(set(ids)),"id_mapping logical_ids must be unique"); need(len(offsets)==len(set(offsets)),"id_mapping offsets must be unique")
    fields(p["corruption"],"strategy byte_offset xor_mask","fixture corruption"); need(p["corruption"]["strategy"]=="XOR_SINGLE_BYTE","corruption strategy"); integer(p["corruption"]["byte_offset"],"corruption byte_offset"); integer(p["corruption"]["xor_mask"],"corruption xor_mask",1,255)
...
def verify_fixture_artifact(binding_dir,plan_fixture,binding):
    fields(binding,"fixture_id relative_path byte_length sha256 content_schema consumer_sections","fixture binding")
    ...
    fields(binding["consumer_sections"],"TRANSFORM ID_MAPPING CORRUPT","fixture consumer_sections"); need(binding["consumer_sections"]==CONSUMER_SECTIONS,"fixture consumer/subfixture mapping mismatch")
...
def verify_physical_artifact(binding_dir,plan_copy,binding,populations):
    ...
    for pop_id in plan_copy["population_ids"]:
        need(pop_id in populations,f"unknown population {pop_id}"); count=populations[pop_id]["count"]; integer(count,f"population {pop_id} count"); states.append((pop_id,"EMPTY_NO_AMORTIZATION",None) if count==0 else (pop_id,"POSITIVE",count))
