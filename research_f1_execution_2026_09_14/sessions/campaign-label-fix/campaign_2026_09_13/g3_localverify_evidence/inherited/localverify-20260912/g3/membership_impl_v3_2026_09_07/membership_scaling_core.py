"""Membership-under-scaling: pure core, CANDIDATE v3. IMPLEMENTATION PREPARATION ONLY.

Supersedes v2 (blob sha256 60141d05f2285d268a88d98344770c8cb5d166168a6fa29371a1ea330db0a1e1) and
v1 (707e166c80d9231d089341a804e0acdd8845781fec7560821c67b3f6d0731a00). Both are left byte-unchanged
in their own namespaces.

v3 is a findings-limited answer to the delta closure check on branch
audit/v52-membership-v2-closure-2026-09-07 @ 712412a5947fa56374e05bf3e3a0b075d4578b6d:

  NEW-1  the conformance test reached only 3 of the 7 published fields, so a forbidden Delta/G ratio
         substituted INTO an existing permitted key survived the whole suite. Fixed in the test
         suite, not here: the core was never wrong, the evidence was too narrow.
  NEW-2  a vacuous `... or True` check was reintroduced. Fixed in the test suite.
  NEW-3  build_clusters([], 0) raised a raw ValueError from np.concatenate instead of a named
         DesignViolation. Fixed here.

Governing documents are bound by commit, path and hash in GOVERNING_DOCUMENTS.md beside this file.
They live on `main` and are deliberately NOT copied here; that file removes the ambiguity a
successor would otherwise face on a candidate branch that predates them.
...
HOW THAT ABSENCE IS EVIDENCED. Stated narrowly, because two earlier versions of this paragraph were
found overstated - v1 by the implementation review (F-4) and v2 by the delta closure check (NEW-1).
No claim is made that any wrong computation is necessarily caught.

What is actually checked, and only this:

  * all SEVEN published fields of `aggregate` - the three scalars, the three per-seed vectors and the
    slot count - are recomputed by an independent reference implementation written from the design
    text in plain Python and compared at the accepted numeric tolerance. Bitwise equality is not
    required, per binding section 6;
  * the output key set is compared against a closed expected set, which catches a quantity added
    under a NEW name;
  * an AST enumeration of divisions and output keys, offered as a supporting check and explicitly
    NOT a proof: a `np.divide` call is not a `Div` node, so this check alone can be walked around;
  * a named list of fault variants in `scripts/mutation_probes_v3.py`, each demonstrated to be caught.

A word scan is not used as assurance anywhere.
...
# a quantity added under a NEW name fails a test rather than passing a word scan. NEW-1 corrects the
# v2 wording here: this constrains key NAMES only. A wrong value - including a ratio - substituted
# INTO one of these permitted keys is invisible to it, and is caught by the conformance test over
# the values instead, which is why that test now covers all seven fields.
...
def build_clusters(cluster_of_question, n_questions: int):
    ...
    if n_questions <= 0:                                                         # NEW-3
        # v2 raised a raw ValueError here, from np.concatenate on an empty list at the partition
        # proof. That is the same defect class F-8 fixed for index selections: a named violation, not
        # a library exception from three frames deeper.
        raise DesignViolation(
            f"cluster grouping needs at least one question, got n_questions={n_questions}")
