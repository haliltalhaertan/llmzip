"""Corpus ingestion v4 for the V52 membership-under-scaling experiment - WRITTEN, NOT AUTHORIZED TO RUN.
...
v2 then claimed 'NO value is interpolated into any message raised or printed by this module', and that
was falsified too - nine more paths, one of them reading its values out of a JSON file the pipeline
itself writes. v3 claimed every message is built by `errors.message`; false again, because the
identifier validators were still elsewhere. Three universal sentences, three falsifications. v4 writes
no fourth.

What is stated instead is an INVENTORY of what was checked:

  * message building - the configuration, manifest, seed, cohort, evidence and output paths of this
    module build through `errors.py`, which admits numbers, booleans, None and enum members validated
    against a closed set;
  * `safe_report` is used where a value must be DESCRIBED rather than named - type, shape, a 12-hex
    digest - and never reproduces the value;
  * error paths exercised by the tests: the nine of F12-F20, the two the v3 closure check found, and
    every evidence-normalisation refusal added in v4. For each, the captured surface is the exception
    message, its repr, the `__cause__`/`__context__` chain, stdout, stderr and every file written.

Untested and therefore unclaimed: what a caller does with what it is handed. Text IS held in memory and
returned - that is what ingestion is for - and a caller that prints it defeats all of the above.
"""
...
import membership_runner_v4 as runner                                            # noqa: E402
...
INGEST_VERSION = "corpus_ingest v4 2026-09-08"
