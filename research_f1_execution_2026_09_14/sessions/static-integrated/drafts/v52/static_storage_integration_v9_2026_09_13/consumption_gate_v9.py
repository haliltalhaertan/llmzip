"""V52 static-storage V9 consumption gate.

Preparation-only, LongMemEval-only. V9 removes V8's stale initial-snapshot bypass:
no verified snapshot is retained by the context. Every consumer-facing access reruns
an exact, pinned V8 chain and returns only that fresh immutable snapshot.
"""
...
class V9ValidationError(ValueError):
    pass

def _need(c, m):
    if not c:
        raise V9ValidationError(m)
...
def _read_regular(path, limit, where):
    ...
    try:
        ...
        raise V9ValidationError(f"{where}: cannot stat source: {exc.__class__.__name__}") from exc
    ...
    try:
        ...
        raise V9ValidationError(f"{where}: cannot open source: {exc.__class__.__name__}") from exc
    try:
        post = os.fstat(fd)
        ...
        while True:
            try:
                ...
                raise V9ValidationError(f"{where}: source read failed: {exc.__class__.__name__}") from exc
            if not chunk:
                break
            total += len(chunk)
...
def _exec_exact(name, path, expected_sha256, dependencies=()):
    ...
    private = f"_v52_v9_{name}_{next(_COUNTER)}"
    mod = types.ModuleType(private)
    mod.__file__ = str(Path(path).resolve())
    mod.__package__ = ""
    restore = {}
    keys = [private]
    for dep_name, dep_mod in dependencies:
        keys.append(dep_name)
        restore[dep_name] = sys.modules.get(dep_name)
        sys.modules[dep_name] = dep_mod
    restore[private] = sys.modules.get(private)
    sys.modules[private] = mod
    try:
        exec(compile(raw, mod.__file__, "exec", dont_inherit=True), mod.__dict__)
    except Exception as exc:
        raise V9ValidationError(f"{name}: exact source execution failed: {exc.__class__.__name__}: {exc}") from exc
    finally:
...
def _load_v8_exact():
    rt = _exec_exact("v8_runtime", _V8_RUNTIME_PATH, V8_RUNTIME_SHA256)
    snap = _exec_exact("v8_snapshot", _V8_SNAPSHOT_PATH, V8_SNAPSHOT_SHA256, (("v8_runtime", rt),))
    gate = _exec_exact(
...
def _fresh_v8(plan_path, plan_sha, fixture_path, fixture_sha, physical_path, physical_sha):
    ...
    try:
        ...
        raise V9ValidationError(f"pinned V8 fresh preflight refused: {exc.__class__.__name__}: {exc}") from exc
