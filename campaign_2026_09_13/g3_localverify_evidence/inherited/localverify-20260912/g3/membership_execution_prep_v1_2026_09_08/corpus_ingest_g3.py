"""Corpus ingestion adapters: Codex v5 implementation preparation.

NOT AUTHORIZED TO RUN ON REAL DATA; independent review remains pending.
The legacy v4 filename is retained for import compatibility. Corpus entry points retain
the inherited real-data gate and accepted source identity checks. LoCoMo evidence follows
the bound producer's accepted normalization semantics; unsupported items and structures
are refused, with raw-item, declared-reference and resolved-reference counts distinguished.
LongMemEval uses per-turn gold markers and does not invent declared-reference counts.

Returned in-memory structures include source text. Persisted summaries use the inherited
schema and structural content policy, which bounds string length and types but cannot
establish provenance or exclude arbitrary short content. Code-built refusals use errors.py;
filesystem and library exceptions remain outside that formatting interface. Synthetic
tests support only their named captured cases, not a universal confidentiality claim.
"""
...
import membership_runner_g3 as runner                                            # noqa: E402
...
EVIDENCE_MODEL_DESCRIPTION = {
    "declared_reference_ids": ("each question declares evidence ids that must resolve to rows of its own "
                               "conversation archive, so a reference CAN dangle and is counted. Raw items, "
                               "normalised references and resolved references are three different numbers "
                               "and are published as three fields"),
...
INGEST_VERSION = "corpus_ingest G-3 remediation 2026-09-11"
...
# --------------------------------------------------------------------------------------------
def _manifest_n_questions(mapping):
    """Return the exact builtin count or refuse before it reaches a diagnostic."""
    value = mapping.get("n_questions") if isinstance(mapping, dict) else None
...
def assert_content_free(payload, _path="payload") -> None:
    """Enforce a structural limit on payload values; this is not a provenance check.
...
# The evidence shapes the BOUND source contract defines, transcribed from the frozen producer's
# `norm_evidence` (research/v52/locomo_sign_mechanism_replication.py @ 692f599e, lines 107-125):
#
#   VALID EMPTY        `None`, and an empty list or tuple. Both mean "this question declares no
#                      evidence". Distinct from a loss - nothing was dropped.
#   VALID NON-EMPTY    a string, from which `D<n>:<n>` ids are extracted; if it contains none, the
#                      producer treats the WHOLE STRING as one id, and that fallback is kept because it
#                      is the bound contract, not a bug to fix here.
#                      A list or tuple whose items are such strings, or dicts carrying `dia_id` or `id`.
#
# Everything else was DROPPED SILENTLY by the producer and by v1, v2 and v3 alike - a dict without
# either key, an empty or null id, a list item that is neither string nor dict, or a top-level value
# that is not None, str, list or tuple. Those never entered the declared count, so `declared` equalled
# `resolved` and a smaller-than-declared gold set looked complete. That is the v3 closure check's
# residual, and it is the one thing this function changes.
#
# Two refusals, kept apart because they are different faults:
#   E-COH-011  a MALFORMED ITEM inside an otherwise valid list - the structure is right, one entry is not;
#   E-COH-012  an UNSUPPORTED STRUCTURE - the field itself is not a shape the contract defines.
# Unsupported shapes are refused. Accepted dict IDs retain the producer's str() conversion.

def _normalise_evidence(value) -> tuple[list[str], int]:
    """Return (reference ids, raw item count). Refuses rather than discarding silently.

    THREE COUNTS ARE KEPT DISTINCT, and this function produces the first two:
      raw items          - how many entries the source presented;
      normalised refs    - how many reference ids they yield, after the producer's de-duplication;
      resolved refs      - how many of those resolve to an archive row. Computed by the caller.
    De-duplication of VALID repeats is unchanged: `dict.fromkeys` as before, so two identical ids still
    collapse to one reference. Only the silent DROPPING of unusable entries is removed.
    """
    if value is None:
        return [], 0                                                     # valid empty
    if isinstance(value, str):
        found = _DIA_ID.findall(value)
        return (found if found else [value]), 1
    if isinstance(value, (list, tuple)):
            ...
            if isinstance(item, str):
                ...
                if not did:
                    errors.raise_violation(DesignViolation, errors.Code.EVIDENCE_ITEM_MALFORMED,
                                           ...
                                           has_dia_id_key="dia_id" in item, has_id_key="id" in item,
                                           id_is_empty=True)
                out.append(str(did))
            else:
                errors.raise_violation(DesignViolation, errors.Code.EVIDENCE_ITEM_MALFORMED,
        ...
        return list(dict.fromkeys(out)), len(value)
    errors.raise_violation(DesignViolation, errors.Code.EVIDENCE_STRUCTURE_UNSUPPORTED,
                           is_none=False, is_str=False, is_sequence=False)
...
def ingest_locomo(source_path, mapping, *, enabled: bool | None = None) -> dict:
    ...
    # Resolve every BOUND id individually. Missing and duplicated bound ids are refused;
    # source questions outside the bound cohort are not selected.
    bound = mapping["expected_question_to_cluster"]
    missing, misrouted, unresolved_position = [], [], []
    empty_gold, partial_loss = [], []
    evidence_tally = {"raw_items": 0, "declared": 0, "resolved": 0, "unresolved": 0}
    for qid, expected_cluster in bound.items():
        ...
        declared, raw_items = _normalise_evidence(question.get("evidence"))
        resolved, unresolved = [], []
        for e in declared:
        ...
        evidence_tally["raw_items"] += raw_items
    ...
    manifest_n_questions = _manifest_n_questions(mapping)
    if resolved != len(bound) or resolved != manifest_n_questions:
        errors.raise_violation(DesignViolation, errors.Code.COHORT_SIZE_MISMATCH,
                               resolved=resolved, cohort_size=len(bound),
                               manifest_n_questions=manifest_n_questions)
    ...
    return {"benchmark": runner.LOCOMO, "identity": identity, "mapping": mapping,
            ...
            "evidence_accounting": {"model": EVIDENCE_MODEL[runner.LOCOMO],
                                    # Three distinct concepts, published as three fields. v3 published
                                    # two and let the third be inferred, which is how a dropped raw
                                    # item could look like an item that never existed.
                                    "raw_evidence_items": evidence_tally["raw_items"],
...
def ingest_longmemeval(source_path, mapping, *, enabled: bool | None = None) -> dict:
    ...
    manifest_n_questions = _manifest_n_questions(mapping)
    if len(questions) != manifest_n_questions:
        errors.raise_violation(DesignViolation, errors.Code.COHORT_SIZE_MISMATCH,
