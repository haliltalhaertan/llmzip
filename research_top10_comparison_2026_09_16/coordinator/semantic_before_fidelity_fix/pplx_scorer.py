...
def official_int8(pooled: np.ndarray) -> np.ndarray:
    ...
    return np.clip(np.round(127.0 * np.tanh(x)), -128, 127).astype(np.int8)
...
def masked_mean_pool(hidden: np.ndarray, mask: np.ndarray) -> np.ndarray:
    ...
    denom = m.sum(axis=1, keepdims=True)  # [B,1]
    denom_safe = np.where(denom == 0, 1.0, denom)
    pooled = (h * m[:, :, None]).sum(axis=1) / denom_safe
    pooled = np.where(denom == 0, 0.0, pooled)
